...
// Returned by mem::export instead of ExportData when the serialized dump
// would not survive the trip back to the engine. The engine's WebSocket
// transport drops the worker connection on frames past 16 MiB, and the
// drop takes every endpoint down with it for the ~1s it takes to
// re-register — so an oversized export has to be refused at the source
